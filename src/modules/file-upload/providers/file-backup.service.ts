...
export class FileBackupService {
  ...
  private async executeBackupStrategy(strategyName: string): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        `Backup strategy ${strategyName} completed. ` +
          `Backed up: ${backedUpCount}, Errors: ${errorCount}`,
      );
  ...
  private async backupFile(
    ...
    if (strategy.includeVariants && file.variants) {
      for (const [variantName, variantUrl] of Object.entries(file.variants)) {
        ...
        if (variantKey) {
          const backupVariantKey = this.generateBackupKey(
            file,
            strategy,
            variantName,
          );
    ...
    if (file.thumbnailUrl) {
      ...
      if (thumbnailKey) {
        const backupThumbnailKey = this.generateBackupKey(
          file,
          strategy,
          'thumbnail',
        );
  ...
  private async cleanupOldBackups(strategy: BackupStrategy): Promise<void> {
    ...
    this.logger.log(
      `Cleaning up backups older than ${strategy.retentionDays} days for strategy ${strategy.name}`,
    );
  }
  ...
  async optimizeStorage(): Promise<void> {
    ...
    try {
      ...
      for (const file of oldFiles) {
        try {
          await this.storageService.changeStorageTier(
            file.storageKey,
            StorageTier.WARM,
          );
          ...
          this.logger.error(
            `Failed to optimize storage for file ${file.id}:`,
            error,
          );
        }
      }
      ...
      const veryOldFiles = await this.fileRepository
        ...
        .andWhere('file.storageTier = :warmTier', {
          warmTier: StorageTier.WARM,
        })
      ...
      for (const file of veryOldFiles) {
        try {
          ...
          this.logger.error(
            `Failed to move file ${file.id} to cold storage:`,
            error,
          );
        }
      }
