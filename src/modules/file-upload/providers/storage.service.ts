...
export class StorageService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.bucketName =
      this.configService.get<string>('AWS_S3_BUCKET') || 'logiquest-files';
  ...
  async uploadFile(
    ...
    const storageClass = this.getStorageClass(
      options.storageTier || StorageTier.HOT,
    );
  ...
  private async updateFileTags(
    ...
    try {
      ...
      const putTagsCommand = new PutObjectTaggingCommand({
        ...
        Key: key,
        Tagging: {
          TagSet: Object.entries(updatedTags).map(([Key, Value]) => ({
            Key,
            Value,
          })),
        },
