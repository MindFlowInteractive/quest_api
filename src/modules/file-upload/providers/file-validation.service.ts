...
export class FileValidationService {
  ...
  private async validateFileSize(
    ...
    if (fileSize > limit) {
      result.errors.push(
        `File size ${fileSize} exceeds maximum allowed size ${limit}`,
      );
      return;
    }
      ...
      if (detected) {
        ...
        if (declaredMimeType !== detected.mime) {
          result.warnings.push(
            `Declared MIME type (${declaredMimeType}) differs from detected type (${detected.mime})`,
          );
  ...
  private async validateMimeType(
    ...
    if (!isAllowed) {
      result.warnings.push(
        `MIME type ${mimeType} is not in the standard allowed list`,
      );
    }
  }
  ...
  private async performSecurityChecks(
    ...
    if (await this.isPolyglotFile(buffer)) {
      result.warnings.push(
        'File appears to be a polyglot (valid in multiple formats)',
      );
  ...
  private async checkFileIntegrity(
    ...
    try {
      ...
      if (!result.warnings.find((w) => w.includes('checksums'))) {
        result.warnings.push(
          `Checksums generated: MD5=${md5.substring(0, 8)}..., SHA256=${sha256.substring(0, 8)}...`,
        );
      }
    } catch (error) {
      result.warnings.push(
