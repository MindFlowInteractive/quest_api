        ...
        if (uploadDto.compress || uploadDto.generateThumbnail) {
          ...
          if (processingResult.success) {
            ...
            if (processingResult.variants) {
              ...
              for (const [variantName, variantBuffer] of Object.entries(
  ...
  async getUserFiles(
    ...
    if (query) {
      queryBuilder.andWhere(
        '(file.originalName ILIKE :query OR file.fileName ILIKE :query)',
        { query: `%${query}%` },
      );
    }
    ...
    if (startDate) {
      queryBuilder.andWhere('file.createdAt >= :startDate', {
        startDate: new Date(startDate),
      });
    }

    if (endDate) {
      queryBuilder.andWhere('file.createdAt <= :endDate', {
