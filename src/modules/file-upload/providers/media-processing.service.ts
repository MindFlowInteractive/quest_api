...
export class MediaProcessingService {
  ...
  private async processImage(
    ...
    try {
      ...
      // Set output format
      const outputFormat = options.outputFormat || this.getOptimalImageFormat(mimeType)
      ...
      for (const variant of this.imageVariants) {
        try {
          let variantImage = sharp(buffer).resize(variant.width, variant.height, {
            fit: variant.height ? "cover" : "inside",
            withoutEnlargement: true,
          })

          if (variant.format === "webp") {
            variantImage = variantImage.webp({ quality: variant.quality || 85 })
          } else {
            variantImage = variantImage.jpeg({ quality: variant.quality || 85 })
          }
