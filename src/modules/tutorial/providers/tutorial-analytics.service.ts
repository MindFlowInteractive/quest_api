...
import { type TutorialAnalytics, AnalyticsEventType } from "../entities/tutorial-analytics.entity"
...
export class TutorialAnalyticsService {
  ...
  async getAnalyticsReport(startDate: Date, endDate: Date, tutorialId?: string): Promise<AnalyticsReport> {
    ...
    const tutorialPerformance = await this.calculateTutorialPerformance(startDate, endDate)

    // Calculate user engagement
    const userEngagement = await this.calculateUserEngagement(startDate, endDate)

    // Generate time series data
    const timeSeriesData = await this.generateTimeSeriesData(startDate, endDate)
  ...
  async getTutorialEffectiveness(tutorialId: string): Promise<{
    ...
    const completionRate = startEvents.length > 0 ? (completionEvents.length / startEvents.length) * 100 : 0
    ...
    const ratings = feedbackEvents.map((event) => event.eventData?.rating).filter((rating) => rating !== undefined)
  ...
  async getUserLearningPattern(userId: string): Promise<{
    ...
    const preferredLearningTime = this.getTimeOfDay(Number.parseInt(preferredHour || "12"))
    ...
    const completionEvents = userEvents.filter((event) => event.eventType === AnalyticsEventType.TUTORIAL_COMPLETED)

    const averageCompletionTime =
      completionEvents.length > 0
        ? completionEvents.reduce((sum, event) => sum + (event.eventData?.timeSpent || 0), 0) / completionEvents.length
        : 0
    ...
    const errorEvents = userEvents.filter((event) => event.eventType === AnalyticsEventType.ERROR_OCCURRED)

    const hintEvents = userEvents.filter((event) => event.eventType === AnalyticsEventType.HINT_USED)

    const strugglingAreas = this.identifyStrugglingAreas(errorEvents, hintEvents)
  ...
  > {
    ...
    return Object.entries(tutorialStats).map(([tutorialId, stats]: [string, any]) => ({
      tutorialId,
      completionRate: stats.started > 0 ? (stats.completed / stats.started) * 100 : 0,
      averageTime:
        stats.completionTimes.length > 0
          ? stats.completionTimes.reduce((sum: number, time: number) => sum + time, 0) / stats.completionTimes.length
          : 0,
      dropoffRate: stats.started > 0 ? (stats.abandoned / stats.started) * 100 : 0,
    }))
  }
  ...
  private async calculateUserEngagement(
    ...
    const sessionTimes = events
      .map((event) => event.eventData?.timeSpent)
      .filter((timeSpent): timeSpent is number => typeof timeSpent === "number")
    ...
    const returnRate = activeUsers > 0 ? (returningUsers / activeUsers) * 100 : 0
  ...
  private identifyStrugglingAreas(errorEvents: TutorialAnalytics[], hintEvents: TutorialAnalytics[]): string[] {
