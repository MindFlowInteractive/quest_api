...
import { type TutorialSession, SessionStatus } from "../entities/tutorial-session.entity"
...
import { SubmitFeedbackDto, UpdateProgressDto } from "../dto/create-tutorial.dto";
...
export class TutorialProgressService {
  ...
  async startTutorial(
    ...
    // Record analytics
    await this.analyticsService.recordEvent("tutorial_started" as AnalyticsEventType, userId, tutorialId, undefined, savedSession.id, {
      deviceInfo,
    })
  ...
  async updateProgress(userId: string, tutorialId: string, updateDto: UpdateProgressDto): Promise<TutorialProgress> {
    ...
    if (progress.totalSteps > 0) {
      progress.completionPercentage = (progress.completedSteps / progress.totalSteps) * 100
    }

    progress.lastAccessedAt = new Date()

    // Check if tutorial is completed
    if (progress.completedSteps >= progress.totalSteps && progress.status !== ProgressStatus.COMPLETED) {
      progress.status = ProgressStatus.COMPLETED
      progress.completedAt = new Date()

      // Record completion analytics
      await this.analyticsService.recordEvent("tutorial_completed" as AnalyticsEventType, userId, tutorialId, undefined, undefined, {
  ...
  async pauseTutorial(userId: string, tutorialId: string): Promise<TutorialProgress> {
    const progress = await this.getUserProgress(userId, tutorialId)
  ...
  async resumeTutorial(userId: string, tutorialId: string): Promise<TutorialProgress> {
    const progress = await this.getUserProgress(userId, tutorialId)
  ...
  async skipTutorial(userId: string, tutorialId: string): Promise<TutorialProgress> {
    const progress = await this.getUserProgress(userId, tutorialId)
    ...
    // Record analytics
    await this.analyticsService.recordEvent("tutorial_skipped" as AnalyticsEventType, userId, tutorialId, undefined, undefined, {
  ...
  async submitFeedback(userId: string, tutorialId: string, feedbackDto: SubmitFeedbackDto): Promise<TutorialProgress> {
    const progress = await this.getUserProgress(userId, tutorialId)
    ...
    // Record analytics
    await this.analyticsService.recordEvent("feedback_submitted" as AnalyticsEventType, userId, tutorialId, undefined, undefined, feedbackDto)
  ...
  async addBookmark(userId: string, tutorialId: string, stepId: string): Promise<void> {
    const progress = await this.getUserProgress(userId, tutorialId)
    ...
    if (!progress.bookmarks.includes(stepId)) {
      ...
      // Record analytics
      await this.analyticsService.recordEvent("bookmark_added" as AnalyticsEventType, userId, tutorialId, stepId)
    }
  }

  async removeBookmark(userId: string, tutorialId: string, stepId: string): Promise<void> {
    const progress = await this.getUserProgress(userId, tutorialId)
  ...
  async updateAdaptiveData(userId: string, tutorialId: string, adaptiveData: Record<string, any>): Promise<void> {
    const progress = await this.getUserProgress(userId, tutorialId)
  ...
  async getProgressStats(userId: string): Promise<{
    ...
    const completedTutorials = allProgress.filter((p) => p.status === ProgressStatus.COMPLETED).length
    const inProgressTutorials = allProgress.filter((p) => p.status === ProgressStatus.IN_PROGRESS).length
    const totalTimeSpent = allProgress.reduce((sum, p) => sum + p.timeSpentSeconds, 0)
  ...
  private async endCurrentSession(userId: string, tutorialId: string, status: SessionStatus): Promise<void> {
    ...
    if (activeSession) {
      ...
      activeSession.durationSeconds = Math.floor(
        (activeSession.endedAt.getTime() - activeSession.startedAt.getTime()) / 1000,
      )
