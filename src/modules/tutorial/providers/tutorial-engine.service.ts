...
export class TutorialEngineService {
  ...
  async processStepInteraction(
    ...
    try {
      ...
      await this.updateStepProgress(userId, tutorialId, interactionDto, validationResult.success)
  ...
  async getCurrentTutorialState(userId: string, tutorialId: string): Promise<TutorialState> {
    const progress = await this.progressService.getUserProgress(userId, tutorialId)
    if (!progress) {
      throw new Error("Tutorial progress not found")
    }
    ...
    const adaptiveSettings = await this.adaptiveService.getUserAdaptiveSettings(userId, tutorialId)
  ...
  async provideHint(
    ...
    // Record analytics
    await this.analyticsService.recordEvent("hint_used" as AnalyticsEventType, userId, tutorialId, stepId, undefined, {
      hintIndex: hintsUsed,
      hint,
    })
  ...
  async skipStep(
    ...
    // Record analytics
    await this.analyticsService.recordEvent("step_skipped" as AnalyticsEventType, userId, tutorialId, stepId, undefined, {
  ...
  async restartTutorial(userId: string, tutorialId: string): Promise<void> {
    ...
    // Record analytics
    await this.analyticsService.recordEvent("tutorial_restarted" as AnalyticsEventType, userId, tutorialId, undefined, undefined, {
      previousProgress: progress.completionPercentage,
    })
  }

  private async validateInteraction(interactionDto: StepInteractionDto): Promise<{
    success: boolean
    feedback: string
    nextAction: "continue" | "retry" | "hint" | "skip"
    score?: number
    ...
    if (isCorrect === true) {
      ...
      return {
        ...
        nextAction: "retry",
        hints: ["Try looking at the highlighted area", "Remember the previous step"],
      }
    }
  ...
  private async updateStepProgress(
    ...
    if (success) {
      ...
      currentStepProgress.errors.push(`Attempt ${currentStepProgress.attempts}: Incorrect interaction`)
  ...
  private calculateTimeRemaining(tutorial: any, progress: any): number | undefined {
