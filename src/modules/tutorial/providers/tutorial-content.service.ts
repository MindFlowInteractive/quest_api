...
import { type TutorialContent, ContentType, ContentStatus } from "../entities/tutorial-content.entity"
...
export class TutorialContentService {
  ...
  private readonly contentTemplates: Record<string, ContentTemplate> = {
    basic_instruction: {
      ...
      defaultValues: { action: "complete the task", description: "Follow the instructions carefully." },
    },
    interactive_practice: {
      type: ContentType.INTERACTIVE,
      template: "Now it's your turn! {instruction} {hint}",
      variables: ["instruction", "hint"],
      defaultValues: { instruction: "Try the interactive element", hint: "Use the hint if you need help." },
    },
    quiz_question: {
  ...
  async validateContent(contentId: string): Promise<ContentValidation> {
    ...
    if (content.content && content.content.length > 5000) {
      validation.warnings.push("Content is very long, consider breaking it into smaller steps")
    }

    // Content type specific validation
    switch (content.contentType) {
      case ContentType.INTERACTIVE:
        if (!content.interactiveElements || Object.keys(content.interactiveElements).length === 0) {
          validation.warnings.push("Interactive content should have interactive elements defined")
        }
        break
      ...
      case ContentType.AUDIO:
        if (
          !content.mediaUrls ||
          (content.contentType === ContentType.VIDEO && !content.mediaUrls.videos) ||
          (content.contentType === ContentType.AUDIO && !content.mediaUrls.audio)
        ) {
          validation.errors.push(`${content.contentType} content requires media URLs`)
          validation.isValid = false
        }
        break
    ...
    if (content.contentType === ContentType.IMAGE && !content.accessibility?.altText) {
      validation.warnings.push("Images should have alt text for accessibility")
    }

    if (content.contentType === ContentType.VIDEO && !content.accessibility?.captions) {
    ...
    if (content.content) {
      ...
      if (wordCount < 10) {
        ...
        validation.suggestions.push("Consider breaking this into multiple steps")
      }

      // Check for action words
      const actionWords = ["click", "select", "choose", "enter", "type", "drag", "drop"]
      const hasActionWords = actionWords.some((word) => content.content.toLowerCase().includes(word))

      if (!hasActionWords && content.contentType === ContentType.INTERACTIVE) {
        validation.suggestions.push("Interactive content should include clear action instructions")
      }
    }
  ...
  async publishContent(contentId: string): Promise<TutorialContent> {
    ...
    if (!validation.isValid) {
      throw new Error(`Cannot publish content with errors: ${validation.errors.join(", ")}`)
    }
...
async duplicateContent(
  ...
  const originalContent = await this.contentRepository.findOne({ where: { id: contentId } });
  ...
  const duplicatedContent = this.contentRepository.create({
    ...(targetTutorialId ? { tutorialId: targetTutorialId } : tutorialId ? { tutorialId } : {}),
  ...
  async getContentByTutorial(tutorialId: string, language = "en"): Promise<TutorialContent[]> {
