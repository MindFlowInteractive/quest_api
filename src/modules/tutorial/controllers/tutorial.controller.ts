...
import { CreateContextualHelpDto, CreateTutorialDto, CreateTutorialStepDto, StartTutorialDto, StepInteractionDto, SubmitFeedbackDto, TutorialSearchDto, UpdateProgressDto } from "../dto/create-tutorial.dto";
...
export class TutorialController {
  ...
  async getTutorial(@Param("id") id: string, @Req() req: Request) {
    ...
    const { tutorial, progress } = await this.tutorialService.getTutorialWithProgress(id, userId)
  ...
  async updateTutorial(@Param("id") id: string, @Body() updateDto: Partial<CreateTutorialDto>) {
  ...
  async addStepToTutorial(@Param("id") tutorialId: string, @Body() stepDto: CreateTutorialStepDto) {
    const step = await this.tutorialService.addStepToTutorial(tutorialId, stepDto)
  ...
  async startTutorial(@Param("id") tutorialId: string, @Body() startDto: StartTutorialDto, @Req() req: Request) {
    const userId = (req.headers["user-id"] as string) || "mock-user-id"

    // Check prerequisites
    const { canStart, missingPrerequisites } = await this.tutorialService.checkPrerequisites(tutorialId, userId)
    ...
    const { progress, session } = await this.progressService.startTutorial(userId, tutorialId, startDto.deviceInfo)

    const tutorialState = await this.engineService.getCurrentTutorialState(userId, tutorialId)
  ...
  async processInteraction(
    ...
    const result = await this.engineService.processStepInteraction(userId, tutorialId, interactionDto)
  ...
  async getHint(@Param("id") tutorialId: string, @Query("stepId") stepId: string, @Req() req: Request) {
    const userId = (req.headers["user-id"] as string) || "mock-user-id"

    const hintResult = await this.engineService.provideHint(userId, tutorialId, stepId)
  ...
  async skipStep(@Param("id") tutorialId: string, @Query("stepId") stepId: string, @Req() req: Request) {
    const userId = (req.headers["user-id"] as string) || "mock-user-id"

    const result = await this.engineService.skipStep(userId, tutorialId, stepId)
  ...
  async pauseTutorial(@Param("id") tutorialId: string, @Req() req: Request) {
    ...
    const progress = await this.progressService.pauseTutorial(userId, tutorialId)
  ...
  async resumeTutorial(@Param("id") tutorialId: string, @Req() req: Request) {
    ...
    const progress = await this.progressService.resumeTutorial(userId, tutorialId)
    const tutorialState = await this.engineService.getCurrentTutorialState(userId, tutorialId)
  ...
  async skipTutorial(@Param("id") tutorialId: string, @Req() req: Request) {
    ...
    const progress = await this.progressService.skipTutorial(userId, tutorialId)
  ...
  async getTutorialProgress(@Param("id") tutorialId: string, @Req() req: Request) {
    const userId = (req.headers["user-id"] as string) || "mock-user-id"
  ...
  async updateTutorialProgress(
    ...
    const progress = await this.progressService.updateProgress(userId, tutorialId, updateDto)
  ...
  async submitFeedback(@Param("id") tutorialId: string, @Body() feedbackDto: SubmitFeedbackDto, @Req() req: Request) {
    const userId = (req.headers["user-id"] as string) || "mock-user-id"

    const progress = await this.progressService.submitFeedback(userId, tutorialId, feedbackDto)
  ...
  async addBookmark(@Param("id") tutorialId: string, @Query("stepId") stepId: string, @Req() req: Request) {
    const userId = (req.headers["user-id"] as string) || "mock-user-id"
  ...
  async removeBookmark(@Param("id") tutorialId: string, @Query("stepId") stepId: string, @Req() req: Request) {
    const userId = (req.headers["user-id"] as string) || "mock-user-id"
  ...
  async getTutorialAnalytics(@Param("id") tutorialId: string) {
    ...
    const effectiveness = await this.analyticsService.getTutorialEffectiveness(tutorialId)
  ...
  async getAnalyticsReport(
    ...
    const report = await this.analyticsService.getAnalyticsReport(start, end, tutorialId)
  ...
  @ApiResponse({ status: 200, description: "Learning pattern retrieved successfully" })
  ...
  @ApiParam({ name: "id", description: "Tutorial ID" })
  @ApiResponse({ status: 200, description: "Adaptive settings retrieved successfully" })
  async getAdaptiveSettings(@Param("id") tutorialId: string, @Req() req: Request) {
    const userId = (req.headers["user-id"] as string) || "mock-user-id"

    const settings = await this.adaptiveService.getUserAdaptiveSettings(userId, tutorialId)
  ...
  @ApiParam({ name: "id", description: "Tutorial ID" })
  @ApiQuery({ name: "stepId", description: "Step ID" })
  @ApiResponse({ status: 200, description: "Personalized content retrieved successfully" })
  async getPersonalizedContent(@Param("id") tutorialId: string, @Query("stepId") stepId: string, @Req() req: Request) {
    const userId = (req.headers["user-id"] as string) || "mock-user-id"

    const content = await this.adaptiveService.generatePersonalizedContent(userId, tutorialId, stepId)
  ...
  @ApiResponse({ status: 201, description: "Contextual help created successfully" })
  async createContextualHelp(@Body() createDto: CreateContextualHelpDto) {
    const help = await this.contextualHelpService.createContextualHelp(createDto)
  ...
  @ApiResponse({ status: 200, description: "Contextual help retrieved successfully" })
  async getContextualHelp(
    ...
    const help = await this.contextualHelpService.getContextualHelp(gameContext, userContext, language)
  ...
  @ApiParam({ name: "strugglingArea", description: "Area where user is struggling" })
  @ApiQuery({ name: "language", required: false })
  @ApiResponse({ status: 200, description: "Smart help retrieved successfully" })
  async getSmartHelp(
    ...
    const help = await this.contextualHelpService.getSmartHelp(userContext, strugglingArea, language)
  ...
  async recordHelpInteraction(
    ...
    @Body() body: { interactionType: "view" | "click" | "dismiss"; viewTime?: number },
    @Req() req: Request,
  ) {
    const userId = (req.headers["user-id"] as string) || "mock-user-id"

    await this.contextualHelpService.recordHelpInteraction(helpId, userId, body.interactionType, body.viewTime)
  ...
  @ApiQuery({ name: "language", required: false })
  @ApiResponse({ status: 200, description: "Localized content retrieved successfully" })
  async getLocalizedContent(
    @Param("id") tutorialId: string,
    @Param("stepId") stepId: string,
    @Query("language") language = "en",
  ) {
    const content = await this.localizationService.getLocalizedContent(tutorialId, stepId, language)
  ...
  async getAvailableLanguages(@Param("id") tutorialId: string) {
    const languages = await this.localizationService.getAvailableLanguages(tutorialId)
  ...
  @ApiParam({ name: "id", description: "Tutorial ID" })
  @ApiResponse({ status: 200, description: "Translation progress retrieved successfully" })
  async getTranslationProgress(@Param("id") tutorialId: string) {
    const progress = await this.localizationService.getTranslationProgress(tutorialId)
  ...
  async createContent(
    ...
    const content = await this.contentService.createContent(tutorialId, contentData.stepId, {
  ...
  async getCategories() {
    ...
    return {
      success: true,
      data: {
        categories: ["onboarding", "puzzle_mechanics", "advanced_strategies", "game_features", "accessibility"],
  ...
  async getPrerequisites(@Param("id") tutorialId: string, @Req() req: Request) {
    ...
    const prerequisites = await this.tutorialService.getPrerequisites(tutorialId)
    const { canStart, missingPrerequisites } = await this.tutorialService.checkPrerequisites(tutorialId, userId)
  ...
  @ApiResponse({ status: 200, description: "Popular help topics retrieved successfully" })
  async getPopularHelpTopics(@Query("gameContext") gameContext?: string) {
    const topics = await this.contextualHelpService.getPopularHelpTopics(gameContext)
  ...
  @ApiParam({ name: "helpId", description: "Help ID" })
  @ApiResponse({ status: 200, description: "Help analytics retrieved successfully" })
